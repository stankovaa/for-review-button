...
<style>
    ...
    /* Expander styling — keep dark background when open */
...
if "graphs_visible" not in st.session_state:
    st.session_state.graphs_visible = False
if "applied_inputs" not in st.session_state:
    st.session_state.applied_inputs = None
...
_apply_clicked = st.sidebar.button(
    "Apply changes",
    type="primary",
...
liq_pref = st.sidebar.selectbox(
    ...
    index=1,
    key="liq_pref",
    help="¹ Liquidation preference multiples are assumed at 1.0x for all preferred share classes. ⁵ Historical investors are assumed to hold shares on equivalent terms to those selected for the new round above. See Model Assumptions for full details.",
)
...
if liq_pref == "Yes":
    pref_type = st.sidebar.selectbox(
        ...
        index=1,
        key="pref_type",
        help="⁴ All investor classes are treated pari passu, with proceeds allocated pro rata to total invested capital.",
    )
...
st.sidebar.markdown('<hr style="margin:4px 0 2px 0; border:none; border-top:1px solid #555;">', unsafe_allow_html=True)
st.sidebar.subheader(f"{last_round} Ownership (%)")
...
with input_col:
    st.text_input(
        ...
        disabled=True,
        label_visibility="collapsed",
        help = "² 'Others' represents the aggregate residual ownership of all remaining cap table participants, including angel investors, ESOPs, advisors, and other minority shareholders. See Model Assumptions for full details.",
    )
...
_current_inputs = dict(
    currency=currency, last_round=last_round, last_post_money=last_post_money,
    raise_goal=raise_goal, pre_money=pre_money, liq_pref=liq_pref,
    pref_type=pref_type, pref_cap=pref_cap,
    founders_pct=founders_pct, historical_pct=historical_pct, new_pct=new_pct,
)
if _apply_clicked:
    st.session_state.applied_inputs = _current_inputs
    st.session_state.graphs_visible = True
elif _current_inputs != st.session_state.applied_inputs:
    st.session_state.graphs_visible = False
...
with st.sidebar.container():
    st.markdown(
        """
        <div style="color: #6b6b6b; line-height: 1.35;">
            <b style="font-size: 13px;">Disclaimer</b><br>
            <span style="font-size: 11px;">This tool is provided for indicative purposes only and does not constitute financial or investment advice. Trachet accepts no liability for decisions made in reliance on this tool — founders are strongly advised to seek independent legal, tax, and financial counsel before pursuing any transaction.</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
verdict = (
        f"Once execution and market risk are priced in, your raise &amp; sell proceeds are worth "
        f"<span class='trachet-highlight'>{currency}{round(risk_adjusted_proceeds / 1e6)}m</span> "
        f"(at post {new_round_name} " 
        f"<span class='trachet-highlight'>{currency}{round(breakeven_raise_sell / 1e6)}m</span>). That is "
        f"<span class='trachet-highlight'>{currency}{round(risk_gap / 1e6)}m</span> "
...
<div class="trachet-callout-row">
  ...
  <div class="trachet-callout">
    <p>
      ...
      (<span class="trachet-highlight">{breakeven_multiple:.1f}x</span> post money {new_round_name}) exit just to match today's value.
    </p>
  </div>
...
if not st.session_state.graphs_visible:
    st.markdown(
        """
        <div style="
            margin-top:40px;
        ...
        ">
            Set your assumptions in the sidebar, then click
            <strong style="color:rgb(0,102,253);">Apply changes</strong>
            to see your results.
        </div>
        """,
        unsafe_allow_html=True,
...
def _make_graphs_pdf(chart_data: dict) -> bytes:
    """Render both charts using matplotlib (no browser required) and return PDF bytes."""
    import io
    import matplotlib
    matplotlib.use("Agg")
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 4.5), facecolor=bg)

    # --- Left chart: Sell Today / Raise & Sell / Baseline / Risk-Adjusted ---
    ax1.set_facecolor(bg)
    ax1.plot(x, chart_data["sell_today"],  color="#d3b0ff", lw=2,        label="Sell Today",          marker="o", markersize=4)
    ax1.plot(x, chart_data["raise_sell"],  color="#2cf4ff", lw=2,        label="Raise & Sell",        marker="o", markersize=4)
    ax1.axhline(chart_data["desired"],     color="#2cfd0d", lw=2, ls="--", label="Baseline Proceeds")
    ax1.plot(x, chart_data["risk_adj"],    color="#ffec00", lw=1, ls=":",  label="Risk-Adjusted",      marker="o", markersize=3)
    ax1.set_title("Founder Proceeds by Exit Valuation", color=textcol, fontsize=10, pad=8)
    ax1.set_xlabel(f"Exit Valuation ({chart_data['currency']}m)", color=textcol, fontsize=8)
    ax1.set_ylabel(f"Founder Proceeds ({chart_data['currency']}m)", color=textcol, fontsize=8)
    ax1.tick_params(colors=textcol, labelsize=7)
    for spine in ax1.spines.values():
        spine.set_edgecolor(gridcol)
    ax1.yaxis.label.set_color(textcol)
    ax1.xaxis.label.set_color(textcol)
    ax1.grid(color=gridcol, linewidth=0.5)
    ax1.legend(fontsize=7, facecolor=bg, edgecolor=gridcol, labelcolor=textcol, loc="upper left")

    # --- Right chart: Difference (R&S vs Baseline) ---------------------------
    ax2.set_facecolor(bg)
    ax2.plot(x, chart_data["difference"],      color="#2cf4ff", lw=2, label="R&S vs Baseline",     marker="o", markersize=4)
    ax2.plot(x, chart_data["difference_risk"], color="#ffec00", lw=1, ls=":", label="R&S vs Risk-Adjusted", marker="o", markersize=3)
    ax2.axhline(0, color="#2a3550", lw=1, ls="--")
    ax2.set_title("Difference: Raise & Sell vs Baseline", color=textcol, fontsize=10, pad=8)
    ax2.set_xlabel(f"Exit Valuation ({chart_data['currency']}m)", color=textcol, fontsize=8)
    ax2.set_ylabel(f"Difference ({chart_data['currency']}m)", color=textcol, fontsize=8)
    ax2.tick_params(colors=textcol, labelsize=7)
    for spine in ax2.spines.values():
        spine.set_edgecolor(gridcol)
    ax2.yaxis.label.set_color(textcol)
    ax2.xaxis.label.set_color(textcol)
    ax2.grid(color=gridcol, linewidth=0.5)
    ax2.legend(fontsize=7, facecolor=bg, edgecolor=gridcol, labelcolor=textcol, loc="upper left")

    fig.tight_layout(pad=1.5)
    buf = io.BytesIO()
    fig.savefig(buf, format="pdf", facecolor=bg, bbox_inches="tight")
    plt.close(fig)
    return buf.getvalue()


def _save_lead_airtable(form: dict, sidebar: dict, chart_data: dict | None = None) -> bool:
    """Create a record in the Airtable 'Form Submissions' table, then attach graphs PDF."""
    try:
        import io
        ...
        if chart_data is not None:
            try:
                pdf_bytes = _make_graphs_pdf(chart_data)
...
if st.session_state.show_email_form:
    ...
    if submitted:
        if not consent:
            ...
            st.warning("Please provide a business email address. Personal email providers, including Gmail, Yahoo, and Outlook, are not supported.")
        else:
            lead = {
            ...
            _save_lead_airtable(lead, {
                ...
                "x":               exit_vals / 1e6,
                "sell_today":      sell_today_vals / 1e6,
                "raise_sell":      raise_sell_vals / 1e6,
                "desired":         desired / 1e6,
                "risk_adj":        risked_vals / 1e6,
                "difference":      difference_vals / 1e6,
                "difference_risk": difference_vals_risk / 1e6,
                "currency":        currency,
            })
