...
def _make_graphs_pdf(fig1, fig3) -> bytes:
    """Render both Plotly figures to PNG via kaleido, combine side-by-side, return PDF bytes."""
    import io
    from PIL import Image

    img1 = Image.open(io.BytesIO(fig1.to_image(format="png", width=650, height=400, scale=2)))
    img3 = Image.open(io.BytesIO(fig3.to_image(format="png", width=650, height=400, scale=2)))

    combined = Image.new("RGB", (img1.width + img3.width, max(img1.height, img3.height)), (11, 18, 32))
    combined.paste(img1, (0, 0))
    combined.paste(img3, (img1.width, 0))

    buf = io.BytesIO()
    combined.save(buf, format="PDF")
    return buf.getvalue()


def _save_lead_airtable(form: dict, sidebar: dict, fig1=None, fig3=None) -> bool:
    ...
    try:
        ...
        if fig1 is not None and fig3 is not None:
            try:
                pdf_bytes = _make_graphs_pdf(fig1, fig3)
